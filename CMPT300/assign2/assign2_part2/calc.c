...
void *adder(void *arg)
{
	...
	for (i = 0; i < bufferlen; i++) 
	{
	    ...
	    if (buffer[i] == '+') 
	    {
	    	...
	    	strncpy(temp_buffer1, &buffer, startOffset);
	    	strcat(temp_buffer1, &result);
	    	
	    	for(int j = 0; j < (bufferlen - remainderOffset) ; j++)
	    		temp_buffer2[j] = buffer[remainderOffset + j + 1];
	    	
	    	strcat(temp_buffer1, temp_buffer2);
	    	strncpy(buffer, temp_buffer1, sizeof(temp_buffer1));
	    	num_ops++;
	    	...
	    	if(isNumeric(buffer[i+1]))
	    	{
	    		if(buffer[i+2] == ')')
	    		{
	    			...
	    			result = buffer[i+1];
  					...
  					strncpy(temp_buffer1, &buffer, startOffset);
	    			temp_buffer1[startOffset] = result;
	    			...
	    			strcat(temp_buffer1, temp_buffer2);
	    			strncpy(buffer, temp_buffer1, sizeof(temp_buffer1));
	    			/*for(int i=0;i<sizeof(temp_buffer1);i++)
	    				printf("buffer:%c\n", buffer[i]);*/
